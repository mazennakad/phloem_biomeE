...
subroutine vegn_Phloem_transport(forcing,vegn)
  ...
  ! Declare variables
  real(8), allocatable :: co(:)    ! Sucrose concentration at the center of the cell
  real(8), allocatable :: uo(:)    ! Longitudinal velocity at the face of the cell
  real(8), allocatable :: po(:)    ! Dynamic fluid pressure at the center of the cell
  real(8), allocatable :: vo(:)    ! Transverse velocity at the center of the cell
  real(8), allocatable :: nuo(:)    ! Dynamic viscosity at the center of the cell
  real(8), allocatable :: Psi(:)    ! Xylem water potential at the center of the cell
  real(8) :: dx, L, a  ! diameter at breast height and tree height
  real(8) :: Resp_s, GResp_s, Growth_s  ! stem demand sucrose g/(ms)
  real(8) :: Resp_r, GResp_r, Growth_r  ! root demand sucrose g/(ms)
  real(8) :: Psi_L, psi0             ! leaf water potential and its scaling
  ...
  do i = 1, vegn%n_cohorts
     ...
     associate ( sp => spdata(cc%species))
       ...
       dx = cc%dbh
       a  = sp%p_thickness
       Resp_s =   cc%DR_stem*Mw_sucr*(1.0e3_8)/(12_8*mol_C) ! g sucrose / step
       Resp_r =   cc%DR_root*Mw_sucr*(1.0e3_8)/(12_8*mol_C)
       GResp_s =   cc%DGR_stem*Mw_sucr*(1.0e3_8)/(12_8*mol_C)
       GResp_r =   cc%DGR_root*Mw_sucr*(1.0e3_8)/(12_8*mol_C)
       Growth_s =   cc%DG_stem*Mw_sucr*(1.0e3_8)/(12_8*mol_C)
       Growth_r =   cc%DG_root*Mw_sucr*(1.0e3_8)/(12_8*mol_C)
       Sstem = (Resp_s + GResp_s + Growth_s)/(3600_8*L*pi*dx) ! g sucrose / m2 s
       Sroot = (Resp_r + GResp_r + Growth_r)/(3600_8*a*pi*dx)
       Sleaf = (Sstem*L + Sroot*a)/a
       Psi_L    = cc%psi_leaf*(1.0e6_8)  ! Pa
       psi0    = - Psi_L

       ! Non-dimensional and scaling quantities
       c0 = psi0 * Mw_sucr*(1.0e3_8) * PI * dx / (Rgas * Tphloem)  ! g/m2
       cw = c0 / (Mw_sucr*(1.0e3_8) * PI * dx) ! mol/m2
       os = Rgas * Tphloem * cw ! Pa
       es = a / L ! dimensionless
       v0 = ks * os ! m/s
       u0 = v0 / es ! m/s
       p0 = L * nu0 * u0 / (a**2) ! Pa
       t0 = (a**2) / D ! s
       G = rho_H2O * 9.8_8 * L / os ! dimensionless
       X0 = psi0 / os ! dimensionless
       Mu = ks * nu0 * (L**2) / (a**3) ! dimensionless
       Pe = a * v0 / D ! dimensionless
       Ss = Sstem / (u0 * c0) ! dimensionless
       Sl = Sleaf / (u0 * c0) ! dimensionless
       Sr = Sroot / (u0 * c0) ! dimensionless


       ! Previouse step variables
       allocate(co(n), uo(n-1), vo(n), po(n),nuo(n), Psi(n))
       ! dimensionless variables
       do j=1,n-1
         co(j) = cc%suc_con(j)/c0
         uo(j) = cc%ax_velo(j)/u0
         vo(j) = cc%trans_velo(j)/v0
         po(j) = cc%phloem_p(j)/p0
         nuo(j) = cc%dy_visco(j)/nu0
       end do
       co(n) = cc%suc_con(n)/c0
       vo(n) = cc%trans_velo(n)/v0
       po(n) = cc%phloem_p(n)/p0
       nuo(n) = cc%dy_visco(n)/nu0

       ! xylem water potential
       call linspace(cc%psi_leaf*(1.0e6_8), cc%psi_stem*(1.0e6_8), n, Psi) ! xylem water potential, linear Pa
       Psi = Psi / psi0 ! dimensiolness
       !write(*,*)"Xylem water potential",Psi*psi0
       !write(*,*)" "
       !write(*,*)"pre",cc%phloem_p(n)
       !write(*,*)" "
       !write(*,*)"con",cc%suc_con(n)
       !write(*,*)" "
       ...
       call iterate(n, dz, Mu, X0, Pe, Sl, Ss, Sr, es, Psi, co, uo, vo, po, nuo, cw, &
        ...
        cc%trans_velo = cc%trans_velo*v0
        cc%phloem_p = cc%phloem_p*p0
        cc%dy_visco = cc%dy_visco*nu0
...
subroutine iterate(n, dz, Mu, X0, Pe, Sl, Ss, Sr, es, Psi, co, uo, vo, po, nuo, &
    ...
    ! Input parameters
    integer, intent(in) :: n
    real(8), intent(in) :: dz, Mu, X0, Pe, Sl, Ss, Sr, es, cw
    real(8), dimension(n), intent(inout) :: co, vo, po, nuo
    real(8), dimension(n-1), intent(inout) :: uo
    real(8), dimension(n), intent(in) :: Psi

    ! Output parameters
    real(8), dimension(n), intent(out) :: ck, vk, pk, nuk
...
subroutine newton(n, dz, Mu, X0, Pe, Sl, Ss, Sr, es, Psi, &
    ...
    integer, intent(in) :: n
    real(8), intent(in) :: dz
    real(8), intent(in) :: Mu, X0, Pe, Sl, Ss, Sr, es, cw
    real(8), intent(in) :: c(n), u(n-1), v(n), p(n), nuDV(n), Psi(n)

    ! Output parameters
    real(8), allocatable, intent(out) :: vector_S(:), Sk(:)

    ! Internal variables
    real(8), allocatable :: PP(:,:), PU(:,:), PV(:,:), PC(:,:), PN(:,:), F1(:)
    real(8), allocatable :: UP(:,:), UU(:,:), UV(:,:), UC(:,:), UN(:,:), F2(:)
    real(8), allocatable :: VP(:,:), VU(:,:), VV(:,:), VC(:,:), VN(:,:), F3(:)
    real(8), allocatable :: CP(:,:), CU(:,:), CV(:,:), CC(:,:), CN(:,:), F4(:)
    real(8), allocatable :: NP(:,:), NU(:,:), NV(:,:), NC(:,:), NN(:,:), F5(:)
    ...
    !real(8), allocatable :: nullvec(:)!p1(:),p2(:),F11(:),F12(:),F21(:),F22(:)
    !real(8), allocatable :: vec(:)!proj, norm2
    ...
    call BuildV(n, dz, v, p, nuDV, u, VP, VU, VV, VC, VN, F3)
    ...
    m(4*n:5*n-1,1:n)       = NP
    m(4*n:5*n-1,n+1:2*n-1) = NU
    m(4*n:5*n-1,2*n:3*n-1) = NV
    m(4*n:5*n-1,3*n:4*n-1) = NC
    m(4*n:5*n-1,4*n:5*n-1) = NN
    ...
    !allocate(vec(5*n-1))
    !vec = 0.0_8
    !vec(1) = (1.0_8/3.0_8)*(1/dz) + Mu
    !vec(2) = - (1.0_8/3.0_8)*(1/dz) + Mu
    !vec(n-1) = (1.0_8/3.0_8)*(1/dz) + Mu
    !vec(n) = - (1.0_8/3.0_8)*(1/dz) + Mu
    !vec(2:n-1) = Mu
    !vec(3*n:4*n-1) = -1
    !m(5*n,1:5*n-1)= vec
    !F(5*n) = -(1/3)*(p(2) - p(1))/dz + sum(Mu*p - c -X0*Psi) - 1/3*(p(n)-p(n-1))/dz
    !F(5*n) = - F(5*n)
    !write(*,*)"residual before",F
    ...
    !allocate(nullvec(5*n - 1))
    !nullvec = 1.0_8
    !proj = 0.0_8
    !norm2 = 0.0_8

    !do i = 1, 5*n - 1
    !  proj = proj + sol(i)*nullvec(i)
    !  norm2 = norm2 + nullvec(i)*nullvec(i)
    !end do

    !do i = 1, 5*n - 1
    !  sol(i) = sol(i) - (proj/norm2)*nullvec(i)
    !end do
    ...
    Sk = Sk + sol
    !write(*,*)"solution",sol
    !totcon = 0.5_8*( Sk(3*n) + Sk(4*n-1) )*dz
    !do i =1,n-2
    !  totcon = totcon + Sk(3*n+i)*dz
    !end do
    !totcon = totcon*cc%height*dz*sp%p_thickness
    !write(*,*)'vector',totcon

    !allocate(p1(n),p2(n),F11(n),F12(n),F21(n-1),F22(n-1))
    !p1 = p
    !p2 = p
    !p1(1) = p1(1) + 1.0e-5_8
    !p2(1) = p2(1) - 1.0e-5_8
    !call BuildP(n, dz, Mu, X0, Psi,c, p1, nuDV, PP, PU, PV, PC, PN, F11)
    !call BuildU(n, dz, u,p1, nuDV, UP, UU, UV, UC, UN, F21)

    !call BuildP(n, dz, Mu, X0, Psi,c, p2, nuDV, PP, PU, PV, PC, PN, F12)
    !call BuildU(n, dz, u,p2, nuDV, UP, UU, UV, UC, UN, F22)

    !write(*,*)"analytical",m(1:2*n-1,1)
    !write(*,*)" "
    !write(*,*)"numerical first",(F11 - F12)/(2.0e-5_8)
    !write(*,*)" "
    !write(*,*)"numerical second",(F21 - F22)/(2.0e-5_8)
    !deallocate(p1,p2,F11,F12,F21,F22)

!    ! Recompute Fk with updated variables
!    call BuildP(n, dz, Mu, X0, Psi, Sk(3*n:4*n-1), Sk(1:n), Sk(4*n:5*n-1), PP, PU, PV, PC, PN, F1)
!    call BuildU(n, dz, Sk(n+1:2*n-1), Sk(1:n), Sk(4*n:5*n-1), UP, UU, UV, UC, UN, F2)
!    call BuildV(n, dz, Sk(2*n:3*n-1), Sk(1:n), Sk(4*n:5*n-1), Sk(n+1:2*n-1), VP, VU, VV, VC, VN, F3)
!    call BuildC(n, dz, Pe, Sl, Ss, Sr, es, Sk(3*n:4*n-1), Sk(n+1:2*n-1), &
!               Sk(2*n:3*n-1), CP, CU, CV, CC, CN, F4)
...
subroutine BuildP(n, dz, Mu, X0, Psi, c, p, nu, PP, PU, PV, PC, PN, F1)
    ...
    F1 = 0.0_8
    do i = 2, n-1
      diagP(i) = -nu(i) * 2.0_8/(3.0_8 * dz**2) - Mu
      uppP(i) = (nu(i+1) - nu(i-1))/(12.0_8 * dz**2) + nu(i)/(3.0_8 * dz**2)
      lowP(i-1) = -(nu(i+1) - nu(i-1))/(12.0_8 * dz**2) + nu(i)/(3.0_8 * dz**2)
      diagN(i) = (p(i+1) - 2.0_8 * p(i) + p(i-1))/(3.0_8 * dz**2)
      uppN(i) = (p(i+1) - p(i-1)) / (12.0_8 * dz**2)
      lowN(i-1) = -uppN(i)
      F1(i) = (nu(i+1) - nu(i-1)) * (p(i+1) - p(i-1)) / (12.0_8 * dz**2) + &
              nu(i)*(p(i+1) - 2.0_8 * p(i) + p(i-1)) / (3.0_8 * dz**2) - &
              Mu*p(i) + c(i) + X0 * Psi(i)
    end do
...
subroutine BuildU(n, dz, u, p, nu, UP, UU, UV, UC, UN, F2)
    ...
    F2 = 0.0_8
    do i = 1, n-1
        UP(i, i) = -(nu(i+1) + nu(i)) / (6.0_8 * dz)
        UN(i, i) = (p(i+1) - p(i)) / (6.0_8 * dz)
        UU(i, i) = 1.0_8
        F2(i) = u(i) + (nu(i+1) + nu(i)) * (p(i+1) - p(i)) / (6.0_8 * dz)
        !if (i < n-1) then
            UP(i, i+1) = (nu(i+1) + nu(i)) / (6.0_8 * dz)
            UN(i, i+1) = (p(i+1) - p(i)) / (6.0_8 * dz)
        !end if
    end do
...
subroutine BuildV(n, dz, v, p, nu, u, VP, VU, VV, VC, VN, F3)
    ...
    integer :: i
      ...
      do i = 2, n-1
          VV(i, i) = 1.0
          VU(i,i-1) = (3.0_8/8.0_8)/dz
          VU(i,i) = - (3.0_8/8.0_8)/dz
          F3(i) = v(i) - (3.0_8/8.0_8)*(u(i) - u(i-1))/dz
      end do
      VV(1,1) = 1.0_8
      VV(n,n) = 1.0_8
...
subroutine BuildC(n, dz, Pe, Sl, Ss, Sr, es, c, u, v, CP, CU, CV, CC, CN, F4)
    ...
    integer, intent(in) :: n
    real(8), intent(in) :: dz, Pe, Sl, Ss, Sr, es
    real(8), intent(in) :: c(n), u(n-1), v(n)
    real(8), allocatable, intent(out) :: CP(:,:), CU(:,:), CV(:,:), CC(:,:), CN(:,:), F4(:)

    ! Internal variables
    integer :: i
    real(8), allocatable :: a1(:), a2(:), a3(:), a4(:), diagC(:), uppC(:), lowC(:)
    ...
    do i = 1, n-1
        a1(i) = ( Pe - (7.0_8/45.0_8)*(Pe**2)*v(i) )*c(i)/dz &
              - (4.0_8/105.0_8)*( (Pe/dz)**2 )* u(i) * (c(i+1) - c(i)) &
              - 2.0_8*(7.0_8/120.0_8)*(Ss/es)*(Pe**2)*dz*(real(i,kind=8))/dz
        a3(i) = - (7.0_8/45.0_8)*(Pe**2) * u(i) * c(i) / dz
    end do

    do i = 1, n-2
        a2(i) = - ( Pe - (7.0_8/45.0_8)*(Pe**2)*v(i) )*c(i)/dz &
               + (4.0_8/105.0_8)*( (Pe/dz)**2 )* u(i) * (c(i+1) - c(i)) &
               + 2.0_8*(7.0_8/120.0_8)*(Ss/es)*(Pe**2)*dz*(real(i,kind=8))/dz
        a4(i) = (7.0_8/45.0_8)*(Pe**2) * u(i) * c(i) / dz
    end do
    ...
    CU(n, n-1) = - ( Pe - (7.0_8/45.0_8)*(Pe**2)*v(n-1) )*c(n-1)/dz &
                 + (4.0_8/105.0_8)*( (Pe/dz)**2 )*u(n-1)*( c(n) - c(n-1) ) &
                 + 2.0_8*(7.0_8/120.0_8)*(Ss/es)*(Pe**2)*dz*(n-1)/dz

    CV(n, n-1) = (7.0_8/45.0_8)*(Pe**2) * u(n-1) * c(n-1) / dz
    !CV(1,1) = 0

    F4 = 0.0_8

    do i = 2, n-1
        diagC(i) = ( Pe - (7.0_8/45.0_8)*(Pe**2)*v(i) )*u(i) / dz &
                 + ((2.0_8/105.0_8)*(Pe**2)*(u(i)**2) + es**2) / (dz**2) &
                 + ((2.0_8/105.0_8)*(Pe**2)*(u(i-1)**2) + es**2) / (dz**2)
        F4(i) = (Pe - (7.0_8/45.0_8)*(Pe**2)*v(i))*u(i)*c(i)/dz &
              - ((2.0_8/105.0_8)*(Pe**2)*(u(i)**2) + es**2) * (c(i+1) - c(i)) / (dz**2) &
              + 2.0_8 * ((Pe*(dz**2)/(2.0_8*es))*(real(i,kind=8)**2) - (7.0_8/120.0_8)*(Pe**2)*dz*u(i)*real(i,kind=8)/es) * Ss / dz &
              - (Pe - (7.0_8/45.0_8)*(Pe**2)*v(i-1))*u(i-1)*c(i-1)/dz &
              + ((2.0_8/105.0_8)*(Pe**2)*(u(i-1)**2) + es**2) * (c(i) - c(i-1)) / (dz**2) &
              - 2.0_8 * ((Pe*(dz**2)/(2.0_8*es))*(real(i-1,kind=8)**2) - (7.0_8/120.0_8)*(Pe**2)*dz*u(i-1)*real(i-1,kind=8)/es) * Ss / dz
    end do

    diagC(1) = Pe*u(1)/dz + ( (2.0_8/105.0_8)*(Pe**2)*(u(1)**2) + es**2 ) / (dz**2)
    diagC(n) = ((2.0_8/105.0_8)*(Pe**2)*(u(n-1)**2) + es**2) / (dz**2)

    ! Boundary conditions for F4
    F4(1) = (Pe - (7.0_8/45.0_8)*(Pe**2)*v(1))*u(1)*c(1)/dz &
          - ((2.0_8/105.0_8)*(Pe**2)*(u(1)**2) + es**2) * (c(2) - c(1)) / (dz**2) &
          + 2.0_8 * ((Pe*(dz**2)/(2.0_8*es)) - (7.0_8/120.0_8)*(Pe**2)*dz*u(1)/es) * Ss / dz &
          - Pe*Sl/dz

    F4(n) = (Pe*Sr + (Pe/es)*Ss)/dz &
          - (Pe - (7.0_8/45.0_8)*(Pe**2)*v(n-1))*u(n-1)*c(n-1)/dz &
          + ((2.0_8/105.0_8)*(Pe**2)*(u(n-1)**2) + es**2) * (c(n) - c(n-1)) / (dz**2) &
          - 2.0_8 * ((Pe*(dz**2)/(2.0_8*es))*(real(n-1,kind=8)**2) - (7.0_8/120.0_8)*(Pe**2)*dz*u(n-1)*real(n-1,kind=8)/es) * Ss / dz

    do i = 1, n-1
        uppC(i) = - ((2.0_8/105.0_8)*(Pe**2)*(u(i)**2) + es**2) / (dz**2)
        lowC(i) = - ( Pe - (7.0_8/45.0_8)*(Pe**2)*v(i) )*u(i)/dz &
                - ((2.0_8/105.0_8)*(Pe**2)*(u(i)**2) + es**2) / (dz**2)
    end do
...
subroutine initialize_cohort_from_biomass(cc,btot,psi_s0,temp)   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!mazen
  ...
  !---- local vars ------------
  integer :: j
  real(8) :: c0, cw, os, es, v0, u0, p0, t0, G, X0, Mu, Pe, totcon
  ...
  associate(sp=>spdata(cc%species))
    ...
    ! Non-dimensional and scaling quantities
    c0 = - cc%psi_leaf*(1.0e6_8)* Mw_sucr*(1.0e3_8) * PI * cc%dbh / (Rgas * Tphloem) ! g/m2
    cw = c0 / (Mw_sucr*(1.0e3_8) * PI * cc%dbh)
    os = Rgas * Tphloem * c0 / (Mw_sucr*(1.0e3_8) * PI * cc%dbh)
    es = sp%p_thickness / cc%height
    v0 = ks * os
    u0 = v0 / es
    p0 = cc%height * nu0 * u0 / (sp%p_thickness**2)
    t0 = (sp%p_thickness**2) / D
    G = rho_H2O * 9.8_8 * cc%height / os
    X0 = - cc%psi_leaf*(1.0e6_8) / os
    Mu = ks * nu0 * (cc%height**2) / (sp%p_thickness**3)
    Pe = sp%p_thickness * v0 / D

    ! xylem water potential
    call linspace(cc%psi_leaf*(1.0e6_8), cc%psi_stem*(1.0e6_8), ng, Psi) ! xylem water potential, linear Pa
    Psi =  Psi / (- cc%psi_leaf*(1.0e6_8))
    call initial(ng, Mu, G, X0, Pe, es, Psi, cw, cc%suc_con, cc%ax_velo, cc%dy_visco, cc%trans_velo, cc%phloem_p)
    !write(*,*)"xylem water potential",Psi*(- cc%psi_leaf*(1.0e6) )
    !write(*,*)" "
    cc%suc_con = cc%suc_con*c0
    !write(*,*)"sucrose", cc%suc_con
    !write(*,*)" "
    cc%ax_velo = cc%ax_velo*u0
    !write(*,*)"Axial velocity", cc%ax_velo
    !write(*,*)" "
    cc%trans_velo = cc%trans_velo*v0
    !write(*,*)"Radial velocity", cc%trans_velo
    !write(*,*)" "
    cc%phloem_p = cc%phloem_p*p0
    !write(*,*)"Dynamic pressure", cc%phloem_p
    !write(*,*)" "
    cc%dy_visco = cc%dy_visco*nu0
    !write(*,*)"Dynamic viscosity", cc%dy_visco
...
subroutine initial(n, Mu, G, X0, Pe, es, Psi, cw, co, uo, nuo, vo, po)
    ...
    ! Input parameters
    integer, intent(in) :: n
    real(8), intent(in) :: Mu, G, X0, Pe, es, cw
    real(8), intent(in) :: Psi(n)

    ! Output arrays
    real(8), dimension(n), intent(out) :: co, nuo, vo, po
...
subroutine guess(n, G, Mu, X0, Psi, cw, co, uo, vo, po, nuo)
    ...
    do i = 1, n
        co(i) = 3.0_8 * (G * dz * (real(i,kind=8) - 0.5_8) - X0 * Psi(i))
        !co(i) = 5.0_8 * ( - X0 * Psi(i))
    end do
...
subroutine VelocityS(co, nuo, Mu, Psi, X0, n, uo, vo, po)
    ! Arguments
    integer, intent(in) :: n
    real(8), intent(in) :: co(n), nuo(n), Mu, X0
    real(8), intent(out) :: uo(n-1), vo(n), po(n)
    real(8), intent(in) :: Psi(n)
    ...
    eqP(n) = -co(n) - X0*Psi(n)
    do i = 2, n-1
        diagP(i) = -nuo(i)*2.0_8/(3.0_8*(dz**2)) - Mu
        uppP(i) = (nuo(i+1) - nuo(i-1))/(12.0_8*(dz**2)) + nuo(i)/(3.0_8*(dz**2))
        lowP(i) = -(nuo(i+1) - nuo(i-1))/(12.0_8*(dz**2)) + nuo(i)/(3.0_8*(dz**2))
        eqP(i) = -co(i) - X0*Psi(i)
    end do
    ...
    do i = 1, n-1
        uo(i) = (nuo(i+1) + nuo(i))*(po(i) - po(i+1))/(6.0*dz)
    ...
    vo = 0.0_8
    do i = 2, n-1
        vo(i) = (3.0_8/8.0_8)*(uo(i) - uo(i-1))/dz
    end do
